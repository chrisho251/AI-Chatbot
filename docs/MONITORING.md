...
- **Services** record metrics with `chatbot_common.metrics`. The api service runs every online component in one process and exposes all their metrics on one `/metrics` route. `create_app` records the duration of every HTTP route, and `ContractClient` records the time of every call between components, in process or over HTTP.
- **Prometheus** scrapes the api service, the model servers, cAdvisor (containers) and the DCGM exporter (GPU) every 15 seconds, and keeps 90 days. Config: `infra/local/prometheus/prometheus.yml`. A worker moved to its own container is added to the `services` job.
- **Postgres `reporting` schema** holds daily views over `ops` and `registry`, and the `ragas_summary` table. It is created by migrations `0002_reporting.py` and `0003_postgres_platform.py`. Grafana reads it with the read-only `grafana_reader` role, which sees aggregates only, never rows about single students.
- **Traces and logs** are optional. The `traces` profile starts the OTel Collector, Loki and Tempo. Set `CHATBOT_OTEL_ENDPOINT` to send to them.
...
- **Performance (Appendix B.1):** latency (time to first token), response time (end to end), throughput by level, output tokens per second, questions in flight, confidence, HTTP p95 and errors per service, and time per worker.
- **Safety and misuse (Appendix B.2):** guard detections by category, stage, action and detector, and blocked questions.
- **Resources (Appendix B.3):** CPU and memory per container (the api service, the sandbox, every model server), GPU utilization and memory, the model server queue, and which targets are up. Online workers share the api container, so their share shows as time per worker on the Performance dashboard.
- **Quality and escalation (Appendix A.6 and C.2):** latest RAGAs scores and their trend, questions per day by level, escalation rate, external and expert response times, tool call verdicts, and knowledge base status.
...
- **`ContractClient` (automatic):** `chatbot_worker_call_seconds` by worker and path, for every call between components.
...
- **Services writing `ops` tables (Lanes A and C):** the daily `reporting` views read them, so nothing extra is needed. The gateway writes `ops.request_facts` and `ops.guard_events`, W5 writes `ops.guard_events` too.
...
- **After adding a component:** add its `HANDLERS` to `chatbot_api.serve`. Nothing changes in Prometheus. After moving a component to its own container: add that container to the `services` job in `prometheus.yml`.
...
- **Retention:** Prometheus keeps 90 days, set in `compose.yaml`. Long-term history lives in Postgres, which the retention sweep manages in place.
...
4. CPU, memory and GPU peaks per container and model server, and the time per worker.
5. RAGAs scores against the targets, with the knowledge base and model versions.
...
- **Loki or Tempo data source errors in Grafana:** they answer only while the optional `traces` profile runs.
